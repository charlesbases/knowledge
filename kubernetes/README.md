...
```shell
# dns 解析
[podName].[serviceName].[namespace].svc.[clusterName].[clusterPort]
...
# 五层
# serviceName.namespace.svc.clusterName.clusterPort
auth-srv.default.svc.cloud.pre

# pod 访问
auth-srv-0.auth-srv.default
```
...
spec:
  ...
  template:
    ...
    spec:
      nodeSelector:
        app: "true"
        kubernetes.io/hostname: "10.64.1.1"
      containers:
      - name: mysql
        image: mysql:8.0.28
        imagePullPolicy: IfNotPresent
...
## 12. [Prometheus](./prometheus/README.md)

------

## 13. [KubeSphere](./kubesphere/README.md)
...
```
  ...
  - ##### kubelet
    ...
    ```shell
    sudo sh -c "cat $(systemctl status -l kubelet | grep Drop-In | awk '{print $2}')/10-kubeadm.conf | grep EnvironmentFile"
    
    sudo vim /etc/default/kubelet
    ...
    KUBELET_EXTRA_ARGS=--root-dir=/mnt/kubelet
    ...
    ```
    ...
    # 查看 kubelet 服务配置
    cat /etc/systemd/system/kubelet.service.d/10-kubeadm.conf | grep EnvironmentFile
    ...
